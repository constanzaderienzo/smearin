...
private:
    ...
    static MStatus cacheInclusiveTransforms(const MDagPath& transformPath, const double startFrame, const double endFrame, std::vector<MMatrix>& inclusiveTransforms);
    static MStatus computeCentroidLocal(const MDagPath& shapePath, const MDagPath& transformPath, MVector & centroidLocal);
    static MStatus computeCentroidTrajectoryWorld(double startFrame, double endFrame, const std::vector<MMatrix>& inclusiveTransforms,
